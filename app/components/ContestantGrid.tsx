              ...
              <AnimatePresence mode="wait">
                ...
                {voteSuccess && (
                  <motion.div
                    ...
                    className="p-8 flex-grow flex flex-col items-center justify-center text-center"
                  >
                    <motion.div
                      initial={{ scale: 0 }}
                    ...
                    <div className="text-gray-400 text-xs md:text-sm leading-relaxed mb-8 space-y-3">
                      ...
                      <p>
                        Ajak teman-temanmu untuk ikut memilih, pemungutan suara
                        akan tetap dibuka hingga{" "}
                        <strong className="text-white">18 Mei 2026.</strong>
                      </p>
                      <p>
                        Pemilih yang beruntung akan mendapatkan total hadiah{" "}
                        <strong className="text-red-500">Rp 16 Juta</strong> dan
                        akan diumumkan pada tanggal 25 Mei 2026 melalui kanal
                        media sosial resmi XPG ADATA.
                      </p>
                    </div>
                    <button
                      onClick={handleCloseVoteModal}
        ...
        {activeGallery && (
          ...
          >
            ...
            >
              ...
              {/* Hapus 'hidden md:block' agar tulisan 'KEMBALI' selalu muncul di HP */}
            ...
            <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex gap-3 z-[10000] max-w-[80vw] overflow-x-auto custom-scrollbar p-2 bg-black/50 backdrop-blur-md rounded-full border border-white/10">
              {activeGallery.images.map((_, i) => (
                <div
                  key={i}
                  className={`w-2.5 h-2.5 rounded-full shrink-0 transition-all ${i === activeGallery.index ? "bg-red-600 scale-125" : "bg-white/30"}`}
                ></div>
              ))}
            </div>
          </motion.div>
        ...
        {activeVideo && (
          ...
          >
            <div className="absolute top-8 right-4 md:top-8 md:right-8 z-[100000] flex gap-2">
              <button
                onClick={() => setActiveVideo(null)}
                className="flex items-center gap-2 bg-[#12141d]/90 hover:bg-red-600 text-white px-5 py-3 rounded-full transition-all backdrop-blur-xl border border-white/20 shadow-[0_10px_25px_rgba(0,0,0,0.5)]"
              >
                <span className="text-xs font-bold uppercase tracking-widest">
                  TUTUP VIDEO
                </span>
