...
export default async function Home() {
    ...
    return (
      <main className="min-h-screen bg-[#050505] flex flex-col items-center justify-center px-4 relative overflow-hidden font-sans">
        <div className="absolute inset-0 z-0">
          <img src="/images/bg-hero.webp" alt="Background" className="w-full h-full object-cover object-top opacity-30" />
        ...
        <div className="relative z-10 text-center flex flex-col items-center max-w-2xl mx-auto mt-[-50px]">
          <img src="/images/mte25-logo.webp" alt="Logo MTE" className="w-[300px] md:w-[450px] mb-8 drop-shadow-[0_0_25px_rgba(220,38,38,0.3)]" />
          ...
          <div className="border border-white/10 bg-white/5 backdrop-blur-sm p-6 md:p-8 rounded-2xl mb-10">
            <p className="text-gray-300 text-base md:text-lg leading-relaxed">
              Mohon maaf, platform voting XPG ADATA PC Modding Contest sedang dalam pemeliharaan rutin untuk sinkronisasi dan peningkatan keamanan data. <br/><br/>
  ...
  const { data: contestants, error } = await supabase.from("contestants").select("*");

  // 2. AMBIL DATA BERSIH (Hanya dari tabel votes_v2 untuk TAMPILAN)
  const { data: cleanVotes } = await supabase.from("votes_v2").select("contestant_id");
  ...
  if (error) {
    return <div className="text-white p-10 text-center">Gagal memuat data.</div>;
  }
  ...
  // =========================================================================
  const displayContestants = contestants?.map(c => ({
    ...c,
  ...
  const leaderboardData = [...displayContestants].sort((a, b) => b.vote_count - a.vote_count);
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white font-sans selection:bg-red-600 selection:text-white">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-4 overflow-hidden pt-20">
        <div className="absolute inset-0 z-0">
          <Image src="/images/bg-hero.webp" alt="Background XPG Mod Battle" fill className="object-cover object-top opacity-30" priority />
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10 w-full flex flex-col items-center">
          ...
          <div className="relative mb-8 drop-shadow-[0_0_25px_rgba(220,38,38,0.3)] flex justify-center">
            <Image src="/images/mte25-logo.webp" alt="Logo Mod To Xtreme 2026" width={600} height={300} className="w-[350px] md:w-[600px] h-auto object-contain" priority />
          </div>
      ...
      <section id="vote-section" className="pt-24 pb-12 px-4 relative z-10 bg-[#050505]">
        <div className="max-w-7xl mx-auto mb-16 text-center">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight">5 Kontestan</h2>
          <p className="text-gray-500 text-lg">Pilih juara favoritmu! Satu suaramu sangat berharga</p>
        </div>
        <ContestantGrid contestants={sortedContestants} />
      </section>

      <section id="judges-section" className="pt-12 pb-24 px-4 relative z-0 bg-[#050505]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="mt-4 text-gray-400 text-sm md:text-base max-w-2xl mx-auto">Indonesian Professional PC modder</p>
          </div>
          <div className="w-[320px] md:w-[400px] mx-auto relative group mt-10">
            ...
            <div className="relative w-full rounded-[1.5rem] md:rounded-[2rem] overflow-hidden transform group-hover:-translate-y-4 group-hover:scale-[1.02] transition-all duration-500 drop-shadow-[0_20px_40px_rgba(220,38,38,0.4)] cursor-crosshair">
              ...
              <Image src="/images/waroq-card.webp" alt="Kartu Ketua Juri WaroQ" width={800} height={1200} className="w-full h-auto object-cover block" priority />
            </div>
          </div>
        </div>
      </section>

      <section id="leaderboard-section" className="py-24 px-4 bg-[#0a0a0a] border-t border-white/5 relative z-0">
        <div className="max-w-4xl mx-auto text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight uppercase">LEADERBOARD</h2>
          <p className="text-gray-500 text-lg">Hasil ini hanya menampilkan jumlah voters yang sudah memenuhi syarat (posting poster bukti voting di Instagram story dan tag @adataxpgindonesia)</p>
        </div>
      ...
      <footer className="bg-[#0a0a0a] border-t border-red-900/30 pt-20 pb-8 px-4 relative z-0">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-12 mb-16">
          <div>
            <div className="mb-6">
              <Image src="/images/mte25-logo.webp" alt="Logo MTE26" width={190} height={40} className="object-contain object-left" priority />
            </div>
          ...
          <div>
            <h4 className="text-red-600 font-bold tracking-widest mb-6 uppercase text-sm">Ikuti Kami</h4>
            <div className="flex flex-col gap-3">
              <a href="https://www.instagram.com/adataxpgindonesia?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==" className="flex justify-between items-center p-4 bg-white/5 border border-white/5 rounded-lg hover:border-red-500/50 hover:bg-white/10 transition-all group">
                <div><p className="text-white font-bold text-sm">Instagram</p><p className="text-gray-500 text-xs mt-1">@adataxpgindonesia</p></div>
              </a>
              <a href="https://www.tiktok.com/@adataxpg.id?is_from_webapp=1&sender_device=pc" className="flex justify-between items-center p-4 bg-white/5 border border-white/5 rounded-lg hover:border-red-500/50 hover:bg-white/10 transition-all group">
                <div><p className="text-white font-bold text-sm">TikTok</p><p className="text-gray-500 text-xs mt-1">@adataxpgindonesia</p></div>
              </a>
              <a href="https://www.facebook.com/share/1DvjkLg1Dz/?mibextid=wwXIfr" className="flex justify-between items-center p-4 bg-white/5 border border-white/5 rounded-lg hover:border-red-500/50 hover:bg-white/10 transition-all group">
                <div><p className="text-white font-bold text-sm">Facebook</p><p className="text-gray-500 text-xs mt-1">ADATAINDONESIA</p></div>
              </a>
            </div>
          </div>
          <div>
            <h4 className="text-red-600 font-bold tracking-widest mb-6 uppercase text-sm">Info Kontes</h4>
            <div className="flex flex-col gap-3">
              <div className="p-4 bg-white/5 border border-white/5 rounded-lg"><p className="text-white font-bold text-sm uppercase tracking-wider">TOTAL HADIAH</p><p className="text-red-500 text-xs mt-1 font-mono tracking-widest">Rp 16.000.000 untuk 32 pemenang</p></div>
              <div className="p-4 bg-white/5 border border-white/5 rounded-lg"><p className="text-white font-bold text-sm uppercase tracking-wider">PESERTA</p><p className="text-gray-400 text-xs mt-1">5 Modders Profesional</p></div>
              <div className="p-4 bg-white/5 border border-white/5 rounded-lg"><p className="text-white font-bold text-sm uppercase tracking-wider">PERIODE VOTING</p><p className="text-gray-400 text-xs mt-1">7-18 Mei 2026</p></div>
              <div className="p-4 bg-white/5 border border-white/5 rounded-lg"><p className="text-white font-bold text-sm uppercase tracking-wider">PENGUMUMAN PEMENANG</p><p className="text-gray-400 text-xs mt-1">25 Mei 2026 di media sosial XPG ADATA</p></div>
              <div className="p-4 bg-white/5 border border-white/5 rounded-lg"><p className="text-white font-bold text-sm uppercase tracking-wider">SYARAT IKUT HADIAH</p><p className="text-gray-400 text-xs mt-1">Upload poster bukti voting di Instagram Story dan tag @adataxpgindonesia. Klaim hadiah maksimal 1 bulan setelah pengumuman pemenang.</p></div>
            </div>
          </div>
        </div>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center pt-8 border-t border-white/5 gap-4">
          <p className="text-gray-600 text-xs tracking-wider text-center md:text-left">© 2026 XPG ADATA Indonesia. Hak cipta dilindungi undang-undang. Kontes PC Modding.</p>
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500 text-xs tracking-widest uppercase font-bold">Sistem Voting Aktif</span>
          </div>
        </div>
      </footer>
    </main>
